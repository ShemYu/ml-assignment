...
REGISTRY_CLUSTER_IP=$(kubectl get svc registry -n kube-system -o=jsonpath='{.spec.clusterIP}'):80
export REGISTRY_CLUSTER_IP
docker build -t $DOCKER_TAG -f $DOCKER_FILE_PATH $DOCKER_BUILD_CONTEXT
docker tag $DOCKER_TAG $REGISTRY_CLUSTER_IP/$DOCKER_TAG
docker push $REGISTRY_CLUSTER_IP/$DOCKER_TAG
